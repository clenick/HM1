...
fetch("toys.php").then(searchResponse).then(toysJson);
...
function search(event){
    fetch("unsplash.php").then(searchResponse).then(searchJson);
...
function send_letter(){ 
    event.preventDefault();
    fetch("takeLetter.php").then(searchResponse).then(checkJson);   
}
function checkJson(json){
    if(json.length>0){
        alert("Hai già inviato una letterina quest'anno!Ritorna l'anno prossimo");
    }
    else{
        const message = document.querySelector("#message");
        let image = image_selected;
        fetch("save_letter.php?image="+image+"&message="+message.value);
        fetch("takeLetter.php").then(searchResponse).then(letterJson);
    }
}
function letterJson(json){
    for(let i=0; i<toys.length; i++){
        fetch("saveRequest.php?letter="+json[0].Id+"&toy="+toys[i]);
    }
}
